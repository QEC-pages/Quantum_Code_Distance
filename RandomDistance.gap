...
ProbabilityCalcLog := function(cols, r, d, i)
    return (- Int(i * Log10(Float(1 / (1 - (Binomial(cols-d, r-1) * d / Binomial(cols, r)))))));
end;
...
# 4-th bit corresponds to the indicating that our program is working. It prints "." each time the the number of iteration rounds is 200*i times for some integer i.
...
DoRandDist := function(arg)
    ...
    local G, num, mindist, F, debug, dims, H, i, j, W, V, dimsW, rows, cols, DistBound, VecCount, FirstVecCount, FirstVecBound, per, W1, W2, TempVec, TempWeight;
    ...
    # FirstVecCount - number of times the first vector, corresponding to the upper bound on the distance, was found.
    # FirstVecBound - first found vector, corresponding to the upper bound on the distance.
    ...
    # so bound is the worst - number of columns in H divided by 2 (because number of qudits is col/2).
	...
	# Inverse permutation
		...
		# Logical operator has to be outside
                if (WeightVecFFE(V * TempVec) > 0) then # linear independence from rows of G
                    if (TempWeight < DistBound) then 
                        ...
                        FirstVecBound := TempVec;; # corresponding vector
			FirstVecCount := 1;; # reset the count of first vector of a such weight
		    ...
		    # If we already received such a weight (up to now - it is minimal), we want to update number of vectors, corresponding to it
                        ...
                        VecCount := VecCount + 1;;
			if TempVec = FirstVecBound then
			    FirstVecCount := FirstVecCount + 1;;
			fi;

			# Debugging (3-rd and 4-th bits of debug): program prints some details or just ".", if the the number of passed iteration rounds is 200*i times for integer i.
			...
			elif ((debug[4] = 1) and (RemInt(i, 200) = 0) and (j = 1)) then
			    Print(".", "\n");  # progress indicator
                        fi;
    ...
    # We check that H*c = 0, that is c is a correct codeword
    if (WeightVecFFE(H * FirstVecBound) > 0) then 
        Print("\n \n", "Failed product weight: ", TempWeight);

	if (Length(FirstVecBound) <= 100) then
	...
	Print("The first vector was found ", FirstVecCount, " time(s). It has a weight (which equals to the upper bound on the distance): ", DistBound, "\n");
	Print(i," rounds of ", num," were made.", "\n");
	Print("The overall number of times vectors with this weight were found: ", VecCount, "\n");
	Print("The probability of existence a vector with lower weight (d-1): 10^(", ProbabilityCalcLog(cols, RankMat(G), DistBound-1, i), ")", "\n");
    fi;
